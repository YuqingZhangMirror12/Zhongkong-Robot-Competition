...
POSITION MapMarkingReturn(POSITION p=car_p)
{
  ...
  if     (MINDISTANCE==storage_dis[EAA])    {MapMarking(p,POS(7,3));CAR_E=EAA;return POS(7,3);}
  else if(MINDISTANCE==storage_dis[EBB])    {MapMarking(p,POS(3,4));CAR_E=EBB;return POS(3,4);}
  else if(MINDISTANCE==storage_dis[ECC])    {MapMarking(p,POS(4,8));CAR_E=ECC;return POS(4,8);}
  else if(MINDISTANCE==storage_dis[EDD])    {MapMarking(p,POS(8,7));CAR_E=EDD;return POS(8,7);}
  else return POS(0,0);
}
#ifdef RUNNING
//动作：从小车当前位置到达仓库位置。
...
delay(1000);
PUT_3();
//MapMarking(POS(9,6),POS(2,6));
//MapTraveling(POS(9,6),POS(2,6));
//delay(1000);
//return_to_storage();
//delay(1000);
//}
